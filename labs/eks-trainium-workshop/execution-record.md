...
## 가속기 동시 관측 — CPU·NeuronCore·요청 대기 분리

2026-09-12 사용자 추가 요청으로 기존 Pod·이미지·모델 설정을 유지하면서 Neuron을 직접 관측했습니다. `neuron-monitor 2.25.145.0`과 이미지에 설치된 AWS 공식 `neuron-monitor-prometheus.py`를 사용했습니다. 관측기는 900초 상한과 stop 파일 종료를 지원하고, 실제로는 결과 회수 시 종료했습니다. Neuron 5초, 새 Prometheus job 5초, 기존 vLLM 지표 10초 수집입니다. 수집기의 부하는 별도 계측하지 않았습니다.

출력 256토큰·동시 요청 4/8을 각 320건, warmup 2건, 조건 사이 30초로 실행했습니다. C4 22:01:20~22:04:26, C8 22:04:56~22:08:02 KST. 각각 320/320 성공. C4 451.54 tok/s·TTFT p95 0.173초·SLO 100%, C8 452.82 tok/s·2.439초·1.25%. 기존 결과와 합치지 않은 별도 1회씩의 비교입니다.

각 실행 시작 +30초부터 종료 −15초까지 Neuron 원본 29개·28개를 집계했습니다. NeuronCore 0/1 평균은 C4 79.29/79.42%, C8 79.55/79.67%. 런타임 디바이스 메모리는 두 조건 모두 약 3.63GiB, CPU 1분 rate 평균 약 0.45코어·최대 약 0.483코어. 대기 요청은 C4 0, C8 4. HPA 목표·가용 Pod는 계속 1이었습니다. raw JSONL 118건과 [집계 결과](./results/2026-09-12-neuron/summary.json), [실제 Grafana 화면](../../articles/screenshots/week6-neuron-c4-c8.png)을 보존했습니다.

CPU가 낮아도 가속기가 동작하고 있다는 것은 확인했습니다. NeuronCore 약 79%를 완전 포화로 해석하거나, 디바이스 메모리 할당량을 HBM 대역폭 사용률로 해석하지 않습니다. `max_num_seqs=4`를 바꾸는 실험과 실제 노드 증설은 수행하지 않았습니다.

추가 관측용 scrape job을 넣을 때 YAML의 `scrape_configs` 밖에 추가해 첫 reload가 거절됐습니다. 기존 Prometheus 설정은 계속 동작했으며, 원인을 파싱으로 확인하고 올바른 위치로 수정한 뒤 exporter `up=1`을 확인하고 측정을 시작했습니다. 결과 회수 후 해당 job만 제거했습니다. 관측 프로세스 종료·9109 포트 닫힘·테스트 Pod 삭제를 확인했습니다. Grafana 대시보드는 완료 시각으로 고정해 보존했습니다. 기존 클러스터·HPA·관측 환경과 root SSH 세션을 유지했습니다.
