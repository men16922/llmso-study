...
## 2026-09-15 — GPU 발표문 간결화와 슬라이드 디자인 조정

- **Changed:** 발표문을 Simple 7장의 순서에 맞춰 구어체로 개정하고, 후속 요청에 따라 장별 세 문단의 핵심 설명·그림 안내와 짧은 Q&A 12개로 정리했습니다. TA 역할과 표준화 → 공동 검토 → 정규 업무 확대 흐름을 유지합니다.
- **Design:** 차콜 표지에 큰 제목과 장식용 GPU 그래픽을 배치하고, 세 핵심 항목의 카드를 흰색으로 통일했습니다. 본문은 흰색, 표·강조 박스는 회색으로 구성하고 블루를 포인트로 사용했습니다. 페이지 번호는 흑백입니다. [현대오토에버 공식 사이트 CSS](https://www.hyundai-autoever.com/common/css/common.css)의 블루 `#1472CF`를 참고하되 발표 화면의 대비에 맞춰 밝기를 조정했습니다. AWS 원본 도식은 그대로 유지했습니다.
- **Verified:** Simple 7장·Detail 15장의 화면·번호/키보드/해시 이동·확대·오프라인·모바일·작은 화면 검사 통과. A4 7/15장 인쇄·텍스트 경계 정상. 원본 이미지와 기존 본문·슬라이드 동작 코드, 상세 근거 파일 보존을 대조했습니다. 발표문 제목 7개와 Q&A 12개 확인. 주요 글자·배경 조합의 명암비 4.5:1 이상, Markdown 인덱스 1,478노드·123문서와 문서 게이트·diff 검사 통과.
- **Next:** 수정본 사용자 검토. 커밋·푸시·외부 업로드 없음.

## 2026-09-14 — TA 검토 범위 상세화와 구축·운영 역할 정의

- **표기 조정:** 사용자 요청으로 최종 영역명을 ‘인프라 설계 / 구축 / 운영’으로 통일했습니다. 두 HTML·상세 근거·발표문에 반영했으며 TA의 역할 정의·설계 검토와 담당 조직의 실제 수행 구분은 유지합니다.

- **Changed:** Simple 2장·Detail 3장의 제목을 ‘TA 검토 범위 상세화’로 변경. 인프라·구축·운영 설계로 검토 항목을 묶고, TA의 구성·기준·담당 역할 정의 및 설계 검토와 구축·운영 조직의 실제 실행을 도식에서 구분했습니다.
- **Aligned:** 두 HTML의 표준화 추진 장표를 표준·역할 정립 → 공동 검토 → 정규 TA 검토 업무 확대로 변경. 초기 협의·POC 착수 문구를 제거하고 표지·역할표·상세 근거 §3·§8·FAQ·발표문 1·2·7장과 Q&A를 같은 업무 수용 흐름으로 맞췄습니다.
- **Verified:** Simple 7장·Detail 15장의 화면·번호/키보드·확대·모바일·인쇄 검사 통과. 검토 범위와 표준화 추진 장표의 도식·역할 문구 확인, 7장 발표문·Q&A 12개 정합성 확인.
- **Next:** 수정본 검토. 커밋·푸시·외부 업로드 없음.

## 2026-09-14 — TA 검토 범위 도식 복원과 설명 구체화

- **Changed:** Simple 2장·Detail 3장을 SageMaker·EKS·HyperPod가 보이던 개요도로 복원. 하단에 인프라 구성 → 모델 배포 → 접근 제어 → 모니터링 → 장애 복구의 확대 방향과 구체적인 검토 질문을 도식화했습니다. 추상적인 부제와 시작 범위·추가 검토 문구를 제거했습니다.
- **Copy:** ‘시험 자료’는 맥락에 맞게 구성도·응답 시간·처리량 측정 결과로 구체화하고, 상세 근거 §3과 발표 스크립트 2장을 같은 설명으로 맞췄습니다.
- **Verified:** Simple 7장·Detail 15장의 화면·번호/키보드 이동·이미지 확대·모바일·인쇄 검사 통과. 두 HTML의 범위 도식과 변경 문구를 확인했고 인쇄 텍스트 경계도 정상입니다.
- **Next:** 수정본 사용자 검토. 커밋·푸시·외부 업로드 없음.
