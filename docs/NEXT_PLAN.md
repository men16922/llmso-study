...
★ **다만 확인이 아니라 반증으로 묻습니다** (2026-09-02 개정). 교재 주장이 맞다면 양자화의 이득은 전부 KV 예산에서 온 것이고, 그렇다면 **예산을 도로 깎으면 이득이 사라져야 합니다.** 그걸 실제로 해봅니다.
...
**잘라내기 순서**: ~~F4~~(잘라냄) → F1c 격자를 c=1·16으로 축소 → F3의 LiteLLM 앞단 → F3 전체 → F2의 Nsight Compute 계층. **F1a·F1b는 자르지 않습니다.**
...
**일정 (2026-09-02 재조정 — 초판은 ⓪ 미착수로 무효)**: 09-02(수) 밤 **⓪ + F1a** / 09-03(목) 밤 **F1b ★ + F1c** / 09-04(금) 밤 **F2** / 09-05(토) 낮 **F3**·그림, 밤 분석·초고 / 09-06(일) 06:00–09:00 윤문·발행·공유. **중단 기준은 설계 문서 §8**(09-05 12:00에 F1b가 안 닫히면 나머지 버리고 F1a+F1b 하나로 간다).
