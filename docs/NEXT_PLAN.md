...
>
> ▶ **3주차 측정 3종(C3·B3·C2) + 글 작성 완료 (2026-08-22).** 남은 것은 **노션 발행 + 링크 공유**뿐입니다. 마감 08-23 09:00.
> 글은 `articles/처리량의 천장은 어디에 있었나.md`, 측정 원본과 분석은 `labs/*/results/`에 있습니다.
...
- [x] [manual] **⑤ 글 작성** — `articles/처리량의 천장은 어디에 있었나.md`. 8단계 템플릿 + `7. 운영 관점`(비용·안정성·확장성·복잡도/관측성·진입비용) + 그림 2종.
- [ ] [manual] **⑥ 노션 발행 + 링크 공유** ★ **남은 것은 이것뿐.** 마감 08-23 09:00. `tools/md_to_notion.py`로 변환, 그림 2장(`fig-c3-layer-*.svg`) 업로드 필요.
