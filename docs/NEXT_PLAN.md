...
>
> ▶ **3주차 진행 방향 확정 (2026-08-16).** 이월해둔 **C3(Ray Serve) + C2(Triton)** 를 3주차 과제로 수행합니다.
> 근거는 아래 「왜 이게 3주차 과제인가」. **C3를 먼저** 끝내 안전판을 만들고 C2를 얹는 순서입니다
> (지난주 실패 패턴 — 네 세션 계획 → 한 세션 수행 → 마감 전날 축소 — 을 반복하지 않기 위함).
...
- [ ] [manual] **① 준비** — Triton 이미지(⚠️ 9.63GB인지 ~17GB인지 확인) + KubeRay·ray-llm ~10GB. **별도 창에서 사람이 직접** (백그라운드 태스크 강제 종료됨)
- [ ] [manual] **② C3 — Ray Serve 배포 + 접점 ① 계층 오버헤드** ★ 안전판. 여기까지만 해도 글 한 편. `rayservice-qwen.yaml`이 B1과 동일 조건이라 기존 결과와 바로 diff
- [ ] [manual] **③ CH5 + 도전과제 2** — `serveConfigV2 → engine_kwargs` 스윕 4행(`max_model_len`·`max_num_seqs`·`util`). ⚠️ **②를 먼저 끝낸 뒤에** 건드릴 것(통제 변수), 원본 매니페스트는 고치지 말 것(`ManifestTest`). **기동 로그 `Maximum concurrency` 줄을 파일로 저장** ★ 2주차에 빠뜨린 증빙
- [ ] [manual] **③-b 도전과제 3 (chunked prefill)** — 여유 시. 잘라내기 1순위
- [ ] [manual] **④ C2 — Triton dynamic batching** — `export_mobilenet_onnx.py --verify`(배치 축 열기)가 진짜 장벽. 대조군은 `0`이 아니라 `off`
- [ ] [manual] **⑤ 글 작성** — 8단계 템플릿, `7. 운영 관점` 필수

**일정**: 08-17 ② / 08-18 ③ / 08-19~20 ④ / 08-21~22 글. ⚠️ 지난주 축소의 원인이 **6일 공백**이었으니 측정을 주중 앞쪽에서 끝낼 것. GPU·8000 포트가 하나라 C3와 C2는 배타적.
