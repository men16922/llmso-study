...
**게이트 green.** `make check` = 문서 검사 3종(`links` / `index` / `tools`) + `labs/` 단위 테스트 **106건**, 전부 오프라인·결정론적, 약 2초.
...
스터디 진행: **1·2·3주차 완료·제출 완료** (3주차 마감 08-23 통과). **4주차(CH7·CH8) — 측정·글·노션 발행 완료, 링크 공유만 남음. 마감 08-30 09:00.**

## Active Focus

Authority: `docs/NEXT_PLAN.md`.

0. **4주차 (CH7·CH8) — ⓪ 선행 + 측정 4종 + 글 + 윤문 + 노션 발행까지 완료 (2026-08-27 한 세션).** 남은 것은 **⑦ 링크 공유뿐이고 이건 사용자가 직접** 해야 합니다. 마감 **2026-08-30 09:00**. 실행 계획은 [`docs/plans/2026-08-27-week4-execution.md`](./plans/2026-08-27-week4-execution.md), 설계는 [`2026-08-23-...`](./plans/2026-08-23-week4-conditional-optimization.md).
1. 글: **`articles/켜면 이득인 최적화는 없다.md`** → 노션 <https://app.notion.com/p/3c94c2420ac48122a485ef00100c6234>. 발행 제목은 **`워크로드 조건이 vLLM 최적화 효과를 바꾸는 방식`**으로 확정. humanize **A**(변경률 24.14%·자체검증 6/6), 로컬·노션 실질 본문 일치. 되읽기로 토글 3·콜아웃 6·이미지 5·표 12·코드 블록 7·수치 멀티셋 전수 대조 — 손실 0.
2. ★ **이번 주 최대 발견** — `prefill` c=64에서 **수용률 100%인데 처리량 −53.7%**. 추측 디코딩의 손해가 "틀려서"가 아니라 **"맞아도 쓸 예산이 없어서"** 임을 분리해 냈습니다. 워크로드별로 수용률을 갈라 재지 않았으면 못 봤을 자리입니다(합산 68.7%는 설명력 0).
3. ★ **draft-0.5B 팔이 그 축을 한 번 더 증명했습니다** (08-28 추가 측정, 글의 **부록 D**). KV 예산 **−42.2%**(140,832 / 34.38x)라 vanilla와 같은 저울에 못 올렸는데, **못 올린 이유가 곧 결과**였습니다 — ngram은 예산을 안 뺏어 `prefill` c=1에서 +199%를 내는데, draft 모델은 **예산 자체를 42% 줄이고 시작해 같은 지점에서 −4.1%**입니다. `decode` c=64 **goodput 0.0%**(e2e p95 45.6초). (재발행 완료.)
3. **AWS GPU 쿼터 신청** — 사용자가 콘솔에서 직접.

## Open Risks

- **과제 미공유 1회 = 제명.** 3주차까지 3회 전부 통과. 4주차 글과 노션 발행은 끝났고 **2026-08-30(일) 09:00 전 링크 공유만 사용자가 직접** 해야 합니다.
