...
스터디 진행: **1·2·3주차 완료·제출 완료** (3주차 마감 08-23 통과). **4주차(CH7·CH8) — 측정·글·노션 발행 완료, ⚠️ 링크 공유 여부 미확인(마감 08-30 09:00 경과).** **5주차(CH9·CH10) 노션 발행까지 완료(09-03), 링크 공유만 남음 — 마감 09-06 09:00.**
...
1. **5주차 (CH9·CH10) — 측정·글·⑥ 노션 발행 완료 (2026-09-03, <https://app.notion.com/p/3d04c2420ac481c89ce1de666fbf9fbe>). 남은 것은 ⑦ 링크 공유(사용자 직접).** 마감 **2026-09-06 09:00**.
   - 글 **[`articles/처리량이 올랐다면 무엇이 빨라진 것인가.md`](../../../articles/처리량이 올랐다면 무엇이 빨라진 것인가.md)** (약 600줄) — 그림 `fig-f1-kv-budget-curve.svg` + **Prometheus 대시보드 캡처 3장 `proof-w5-07~09`**. 터미널 로그를 HTML로 렌더링한 카드 6장(`proof-w5-01~06`)은 **2026-09-03 사용자 지적으로 삭제** — 생성 이미지는 실증 스크린샷이 아닙니다. **2026-09-04 humanize heavy(리포트·적극) 등급 A** — 로컬 원고와 노션 페이지 양쪽 반영 완료.
