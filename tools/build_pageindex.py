...
MD_SKIP_DIRS = {
    ...
    ".venv",
    "venv",
    # pytest가 만드는 캐시. 안에 README.md가 들어 있어 인덱스를 오염시킨다
    # (2026-08-15에 실제로 4개가 들어갔다).
    ".pytest_cache",
    # 스터디 노션 원문을 로컬로 받아둔 자리. 멤버 전용 자료의 전재이므로
    # 커밋하지 않고(.gitignore) 인덱스에도 넣지 않는다 — 인덱스는 커밋되는데
    # 거기에 원문 발췌가 들어가면 사실상 저장소에 전재하는 것이 된다.
    "study",
}
