...
def fig_queue(fname):
    ...
    out = head(
        "처리 중(running)과 대기 중(waiting) 요청 수",
        "vLLM 내부 지표 · max-num-seqs=16 고정 · 동시성 4 → 16 → 64 · 10초 간격",
...
def main():
    ...
    fig_log(
        "fig-b1-ttft.svg",
        "동시성에 따른 TTFT p95 변화",
        "같은 측정 · 로그 눈금 · 처리량 증가가 멈추는 지점부터 TTFT가 급증한다",
    ...
    # ── 엔진에 여유를 준 조건(slots=64) ─────────────────────────────
    # 같은 엔진·같은 설정에서 계층 유무만 다르다. 슬롯이 16일 때는 엔진 자체가
    # 1,200에서 막혀 계층의 몫이 가려졌다. 64로 열어야 계층이 진짜 천장임이 보인다.
    # head()가 색 클래스 s0~s2를 항상 정의하므로 계열이 둘이어도 셋을 채워 둔다
    # (셋째는 쓰이지 않는다 — files를 2개만 넘기기 때문).
    SERIES = [
        ("직접 vLLM", "#2a78d6", "#3987e5"),
        ("Ray Serve", "#1baf7a", "#199e70"),
        ("(미사용)", "#eb6834", "#d95926"),
    ]
    seqs64 = [
        "b-direct-v072-seqs64.json",   # 같은 엔진, 계층 없음
        "b3-seqs64.json",              # 같은 엔진, Ray Serve
    ]
    fig_linear(
        "fig-c3-seqs64-throughput.svg",
        "슬롯을 64로 열었을 때 — 계층이 있고 없고",
