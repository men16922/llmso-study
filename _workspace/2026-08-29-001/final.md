...
> **핵심 결과**
>
> - 추측 디코딩의 처리량은 워크로드와 동시성에 따라 **+199.3%에서 −53.7%까지** 달라졌습니다. 같은 기법도 부하 조건이 바뀌면 이득이 손해로 뒤집혔습니다.
> - 처리량이 **−53.7%** 감소한 구간에서도 수용률은 **100.0%**였습니다. 추측이 모두 맞아도 한 스텝의 토큰 예산이 부족하면 다른 요청의 실행 기회를 줄여 손해가 납니다.
> - 세 기법은 vLLM 스케줄러의 같은 **`token_budget`**에 각각 다른 방식으로 관여합니다. 기능을 켜기 전에 워크로드가 어떤 자원을 쓰는지와 토큰 예산에 여유가 있는지를 먼저 확인해야 합니다.

---

## 목차

- [1. 무엇을 확인하려 했나](#1-무엇을-확인하려-했나)
- [2. 추측 디코딩은 워크로드와 동시성에 따라 결과가 뒤집혔다](#2-추측-디코딩은-워크로드와-동시성에-따라-결과가-뒤집혔다)
- [3. 수용률만으로는 손해를 설명할 수 없었다](#3-수용률만으로는-손해를-설명할-수-없었다)
- [4. 같은 토큰 예산으로 나머지 기법을 해석했다](#4-같은-토큰-예산으로-나머지-기법을-해석했다)
- [5. 세 기법이 이득을 내는 조건](#5-세-기법이-이득을-내는-조건)
- [6. vLLM 스케줄러에서 확인한 공통 원리](#6-vllm-스케줄러에서-확인한-공통-원리)
- [7. 실험의 한계](#7-실험의-한계)
- [8. 워크로드를 확인한 뒤 최적화를 적용한다](#8-워크로드를-확인한-뒤-최적화를-적용한다)
- [부록](#부록)
- [참고 자료](#참고-자료)
...
| 기법 | 하는 일 | 기대 효과 |
|---|---|---|
| **추측 디코딩** | 다음 토큰 여러 개를 먼저 생성하고 본 모델이 한 번에 검증 | 한 스텝에서 여러 토큰 생성 |
| **chunked prefill** | 긴 프리필을 여러 청크로 나눠 디코드와 같은 스텝에 배치 | 프리필과 디코드의 간섭 완화 |
| **prefix caching** | 앞부분이 같은 요청들의 KV를 재사용 | 반복되는 프리필 연산 제거 |
...
<!-- HUMANIZE-SUMMARY
run_id: 2026-08-29-001
mode: fast-sectioned
genre: report — 기술 실측 리포트, 격식체 '-습니다'
metrics:
  char_in: 19661
  char_out: 19597
  change_rate: 24.14%  # Notion 발행본 대비 서식 정규화 후 변경 어절 비율
  self_check: 6/6
  grade: A
categories:  # before → after
  A-10 '~할 수 있다' 남발: 2 → 1
  C-8 기계적 대구 'A가 아니라 B': 12 → 7  # 기술적 대조 7건은 의미 보존을 위해 유지
  C-11 연결어미 뒤 쉼표: 6 → 1  # 이미지 대체 텍스트·표·코드 제외
  D-1 결산 관용구: 0 → 0
  H-1 문두 접속사 남발: 0 → 0
  I-1/I-3 형식명사 결말: 0 → 0
self_check:
  ...
  - 신규 비유·과장·상투구 추가 없음: ✅
residual_s2:
  - C-8: compute-bound/memory-bound, ON/OFF, 원인 A/B처럼 실험 축을 구분하는 대조만 보존
  - J-3: 헤딩 라벨·이미지 대체 텍스트·코드 설명의 대시는 서식 또는 관측 설명이라 보존
highlights:
  - "# 켜면 이득인 최적화는 없다" → "# 워크로드 조건이 vLLM 최적화 효과를 바꾸는 방식"
  - "이번 글은 그 다음 칸입니다." → "이번에는 같은 서빙 구조 안에서 최적화 기법의 효과가 언제 달라지는지 살펴봤습니다."
  - "첫 가설은 뻔했습니다." → "첫 가설은 추측이 자주 틀려서 처리량이 감소한다는 것이었습니다."
  - "이걸 모르면 뒤의 셋은 전부 찍기입니다." → "이 구분이 없으면 세 기능의 효과를 예측하기 어렵습니다."
grade_reason: "A — S1 잔존 0건, S2 잔존 2개 범주, 변경률 24.14%, 자체검증 6/6."
-->
