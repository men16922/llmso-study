...
운영에서 확인할 것은 그 이득을 껐다 켰다 할 수 있는가입니다. 무엇을 켰더니 빨라졌는지는 그다음 질문입니다. 껐는데도 이득이 남는다면 원인을 잘못 짚은 것입니다.
...
[지난 글](./%EC%BC%9C%EB%A9%B4%20%EC%9D%B4%EB%93%9D%EC%9D%B8%20%EC%B5%9C%EC%A0%81%ED%99%94%EB%8A%94%20%EC%97%86%EB%8B%A4.md)에서는 최적화 기능의 이득이 언제 손해로 뒤집히는지를 다뤘는데 그때 조건은 한 스텝의 토큰 예산이었습니다. 이번 질문은 그다음입니다. 이득이 났을 때, 그 이득은 어디서 온 것인가?
...
앞 장까지는 밖에서 본 숫자입니다. 처리량과 ITL은 결과이지 원인의 위치를 알려주지 않습니다. CH10이 제안하는 순서, 곧 지표에서 시작해 타임라인으로 다시 커널로 좁혀 들어가는 순서를 따라 안을 열어 봤습니다.
...
둘째, 줄어든 시간은 전부 선형 계층에 있습니다. GEMM 경로가 −27.7%인데 어텐션·KV 캐시 경로는 −0.1%로 미동도 하지 않았습니다. KV 캐시는 여전히 BF16이고 양자화가 손댄 곳은 가중치를 읽는 대목뿐입니다. 3장에서 예산을 되돌려도 이득이 남았던 이유가 여기서 확인됩니다. 이득은 애초에 KV 쪽에서 나온 적이 없습니다.
...
아래 세 캡처는 모두 같은 25분 창(그래프 시간축 기준 9월 2일 20:48~21:13, UTC)을 보고 있습니다. 시간축에서 21:01·21:05·21:09에 시작하는 세 덩어리가 순서대로 A·B·C이고 구간 사이의 빈 구간은 재배포로 서버를 갈아 끼우던 시간입니다.
...
배운 것은 결론보다 절차 쪽입니다. "조건이 달라졌으니 비교할 수 없다"고 말하려면 그 조건이 실제로 결과를 움직이는지를 먼저 재야 합니다. 재지 않고 내린 배제는 안전해 보이지만 실제로는 정보를 버리는 결정이었습니다.
...
바꿔 말하면 반박한 것은 사슬 자체가 아니라 **사슬을 조건 없이 적용하는 습관**입니다.
...
mode: heavy / targeted (강도: 적극)
metrics:
  char_in: 24909
  char_out: 24010
  change_rate: 12.8%  (자가 산출 — 확정 판정은 오케스트레이터 Phase 2.5)
  self_check: 6/6
  grade: A
categories:  # before → after (산문 기준, 표·코드·헤딩 제외)
  J-1 산문 볼드 강조: 68 → 12   # 문단 머리 볼드 결론 골격 해체, 표·불릿 라벨·핵심 pull-quote만 잔존
  C-4 문단 첫 문장 요약 공식: 21 → 4
  C-8 "A가 아니라 B" 대구: 23 → 8   # 두 문장 분해 / 종속화 / 대조어 제거로 분산, 연속 문단 재발 0
  J-3 산문 em-dash 삽입구: 19 → 0   # 양측 대시 삽입구 4건 전부 해체. 불릿 라벨·참고문헌 구분 대시는 보존
  C-11 연결어미 뒤 쉼표: 41 → 2    # ★S1 (z=+4.72) — 열거 -고/-며 연쇄는 마침표 분리로 처리
  D-5 의인화 추상 주어(본문): 11 → 3   # 헤딩 내 의인화는 앵커 보존을 위해 미수정
  E-1 문장 길이 분산: 짧은 단정문 7건 후행 문장과 병합, 긴 복문 9건 분할
  E-2 어휘 재활용: "그대로" 11 → 4 / "자리"(본문) 8 → 2 / "이 글" 15 → 4
  A-18 관형절 3중 중첩: 12 → 6   # 좌향 수식 후치 전개
  A-7 have/make 직역: 1 → 0     # "FP8 텐서코어를 갖고 있습니다" → "FP8 텐서코어가 있습니다"
  H-4 "즉" 남발: 3 → 0
not_targeted:  # 진단이 금지한 축 — 손대지 않음
  - F-4/F-5 한자어 명사화 (z=-1.80, 이미 낮음)
  - D-1 결산 lexicon / G-3 안전 균형 (카운트 0)
  - A-8/A-9 피동 · A-16 대명사 · A-17 -들 (카운트 0)
  - 쉼표 신규 증가 (comma_usage_rate z=-1.17 — 총량 유지)
byte_preserved:
  - 헤딩 전체(`#`~`###`) 및 목차 링크 11개: 무변경 (앵커 무결)
  - 마크다운 표 20개 전체(셀 내 볼드·정렬줄 포함): 무변경
  - 코드 블록 10개, 이미지 줄 4개, 링크·URL·퍼센트 인코딩 경로: 무변경
  - 수치·단위·퍼센트·모델명·플래그명·지표명·파일명 전량: 무변경
  - 직접 인용 2건(CH9 인용, 4주차 자기 인용): 인용 부호 안쪽 무변경
  - 합쇼체 유지 (하향·상향 모두 없음)
  - 문단 수·순서·장 구성: 동일. 삭제·병합·신설·요약 없음
self_check:
  - 고유명사·수치·인용 100% 보존: OK
  - 변경률 30% 이하 (12.8%): OK
  - 장르 이탈 없음 (기술 리포트 유지): OK
  - register 보존 (합쇼체 양방향 불변): OK
  - S1 잔존 0건 (C-11 잔존 2건은 절 경계 모호 회피용 의도 보존): OK
  - 인공 표현 추가 없음 (없던 주장·상투구 0건, 저자 고유 비유 "저울·팔·쫓아냈습니다·평평한 선" 보존): OK
highlights:
  - id: C-11 + J-1
    before: "**꺾입니다 — 다만 2.48x에서입니다.** 예산이 … 처리량이 **35% 떨어지고** TTFT가 1.7배로 늘었으며, vLLM이 이미 자리를 잡은 요청을 **네 번 쫓아냈습니다**(선점)."
    after: "꺾이기는 합니다. 다만 그 점이 2.48x입니다. 예산이 … 처리량이 35% 떨어지고 TTFT가 1.7배로 늘었으며 vLLM이 이미 들어와 있던 요청을 네 번 쫓아냈습니다(선점)."
  - id: J-3 (양측 대시 삽입구)
    before: "교재가 말한 사슬의 가운데 고리 — *KV 확보 → 배칭 촘촘 → 처리량* — 는 **예산이 이미 남아도는 구간에서는 아무것도 전달하지 않습니다.**"
    after: "교재가 말한 사슬의 가운데 고리, 곧 *KV 확보 → 배칭 촘촘 → 처리량*은 예산이 이미 남아도는 구간에서 아무것도 전달하지 않습니다."
  - id: D-5 (의인화)
    before: "그리고 챕터가 스스로 답을 냅니다 — 양자화로 처리량이 2.7배 올랐는데 …"
    after: "답도 같은 챕터에 적혀 있습니다. 양자화로 처리량이 2.7배 올랐는데 …"
  - id: C-8 (대구 두 문장 분해)
    before: "측정된 ITL은 **0.0097초에서 0.0073초로 24.7% 줄었습니다.** 스텝 수가 준 것이 아니라 스텝 하나가 빨라졌습니다."
    after: "측정된 ITL은 0.0097초에서 0.0073초로 24.7% 줄었습니다. 스텝 수는 줄지 않았습니다. 스텝 하나가 빨라진 것입니다."
  - id: E-1 (짧은 단정문 병합)
    before: "원인은 간단합니다. **두 트레이스가 담은 구간의 길이가 다릅니다.**"
    after: "원인은 두 트레이스가 담은 구간의 길이가 다르다는 데 있습니다."
residual_findings:
  - id: C-11
    severity: low
    count: 2
    reason: "'예산이 남아돌던 A에서도, 대기열이 …' 및 표 캡션 열거 1건 — 쉼표 제거 시 절 경계가 모호해져 의도 보존"
  - id: C-8
    severity: low
    count: 8
    reason: "pull-quote 3건과 정의적 대조 5건은 명제 자체가 이항 대조 — 문형 제거 시 의미 훼손"
  - id: J-1
    severity: low
    count: 12
    reason: "리드 dek 1건, 불릿 라벨 6건, 핵심 수치 강조 5건 — 표·라벨 성격으로 보존 판정"
  - id: C-3 / D-5(헤딩)
    severity: n/a
    reason: "헤딩 문형 균일·헤딩 내 의인화는 목차 앵커 11개 무결 유지를 위해 진단만 하고 미수정 (사용자 지시)"
grade_reason: "A — S1(C-11) 41 → 2, 산문 em-dash 19 → 0, 산문 볼드 68 → 12. 변경률 12.8%로 30% 목표 안쪽이며 자체검증 6항 통과. 헤딩·목차·표·코드·수치·인용 바이트 무변경, 합쇼체와 문단 골격 그대로."
-->
