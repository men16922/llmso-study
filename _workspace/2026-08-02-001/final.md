...
원본 글자수: 27,489
윤문본 글자수: 28,643
변경률: 6.34% (접기 태그와 들여쓰기 등 레이아웃 변경 제외)
카테고리별 탐지: A-10 2→0, C-5 5개 표기→0, D-1 1→0, H-1 1→0
자체검증: 6/6 통과
등급: B — S1 잔존 0건, 보수 윤문으로 A 등급 권장 변경률(10~25%) 미만
주요 변경:
- "출력을 그대로 박제한 기록" → "명령과 출력을 가공하지 않고 남긴 실습 기록"
- "결론부터 적으면" → "실습 결과"
- 기호 중심 헤딩 → 내용이 바로 드러나는 서술형 헤딩
- 원문과 다른 지점 → 관련 장을 표시한 트러블슈팅 토글
- "k3s가 통째로 내려간다" → "k3s가 종료되는 문제"
-->
