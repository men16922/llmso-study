...
## 1. 처리량 증가의 원인을 분리해 측정했다
[지난 글](https://app.notion.com/p/3c94c2420ac48122a485ef00100c6234)에서는 최적화 기능의 이득이 손해로 뒤집히는 조건을 찾았습니다. 기준은 한 스텝의 토큰 예산이었습니다. 이번에는 한 걸음 더 들어가 처리량 증가의 원인을 묻습니다.

CH9도 첫 줄에서 같은 질문을 던집니다.
...
양자화를 켜면 KV 예산과 처리량이 동시에 늘어납니다. ON/OFF 비교만으로는 둘이 원인과 결과인지, 함께 나타난 변화인지 가려낼 수 없습니다. FP8은 그대로 두고 KV 예산만 BF16 수준으로 되돌린 이유입니다.
...
양자화 팔에는 `--quantization fp8`만 추가했습니다. 이 옵션은 BF16 체크포인트를 기동할 때 W8A8로 바꾸므로 새 모델 파일이 필요하지 않습니다. 체크포인트, 이미지, 실행 경로는 같고 가중치 정밀도만 달랐습니다.
...
### 비교가 성립하는지 먼저 확인했다
각 팔의 기동 로그에서 `Maximum concurrency`를 먼저 기록했습니다. 이 값은 정적 공식이 아니라 vLLM의 기동 시 프로파일링 결과라 같은 설정에서도 롤아웃마다 달라질 수 있습니다. 실제 예산을 확인하지 않고 처리량부터 비교하면 통제하지 않은 차이를 기능의 효과로 오해하게 됩니다.

측정 잡음도 먼저 구했습니다. BF16 기준 팔을 세 번 반복했을 때 재현 편차는 **0.38%**(σ 3.3 tok/s)였습니다. 뒤에서 근거로 삼는 **+33.8%**는 이 편차보다 약 90배 큽니다.

---

## 2. KV 예산을 3.8배 늘려도 처리량은 그대로였다
제거 실험에 앞서 KV 예산과 처리량의 관계부터 측정했습니다. 이 관계를 바탕으로 양자화가 만든 이득에서 예산의 몫을 분리했습니다.
...
vLLM은 들어와 있던 요청도 네 번 선점했습니다. 10.15x 이상에서는 다시 평평했습니다.
...
```
GPU 용량은 11.99 GiB지만 기동 시 자유 메모리는 10.79 GiB였습니다. 윈도우 데스크톱이 나머지 1.2 GiB를 사용하고 있었습니다. 이 랩톱에서는 0.85를 상한으로 두고 0.45를 하한에 추가해 스윕 범위를 확보했습니다.
...
교재의 설명대로라면 늘어난 KV 예산이 처리량 증가의 원인이어야 합니다. 이를 확인하려고 FP8의 `gpu_memory_utilization`을 낮춰 `Maximum concurrency`를 BF16 수준으로 맞춘 세 번째 팔을 만들었습니다.
...
```
판정 기준은 실험 전에 정했습니다. ③이 ① 수준으로 내려가면 이득은 메모리에서 왔다고 봅니다. ②와 비슷하면 다른 경로에서 온 것입니다.
<table fit-page-width="true" header-row="true">
<tr>
<td>팔</td>
<td>util</td>
...
4주차의 추측 디코딩은 같은 서버에서도 **+194.8%와 −52.8%** 사이를 오갔습니다. 워크로드와 동시성이 달라지자 이득이 손해로 바뀌었습니다. FP8에서도 같은 반전이 나타나는지 보려고 프리필·디코드 × 동시성 1·4·16·64, 팔마다 2회 측정했습니다.
<table fit-page-width="true" header-row="true">
<tr>
<td>워크로드</td>
<td>동시성</td>
...
3장의 실행 경로가 이 패턴을 설명합니다. 양자화는 스텝마다 다시 읽는 가중치 바이트를 줄이지만 그 비용은 배치 안의 모든 요청이 나눠 냅니다. 동시 요청이 하나일 때는 3.1 GB를 읽어 토큰 하나를 만듭니다. 64개일 때는 같은 3.1 GB로 64토큰을 만듭니다. 배치가 커질수록 전체 시간에서 가중치 읽기가 차지하는 비중과 양자화가 줄일 몫이 함께 작아집니다.
...
정규화·활성화의 −98.6%도 연산이 사라졌다는 뜻은 아닙니다. FP8 커널 목록에는 `triton_red_fused__to_copy_abs_clamp_cutlass_scaled_mm...`이 있습니다. 활성화를 8비트로 낮출 때 필요한 절댓값·클램프·스케일 계산이 정규화와 한 커널로 합쳐졌습니다. 역양자화 비용까지 GEMM 경로에 포함한 결과가 **−27.7%**입니다.

### Ada의 FP8 커널은 교재와 다른 결과를 만들었다
CH9은 Nsight 딥다이브에서 양자화 전후 GEMM 커널 실행시간이 거의 같았다고 적었습니다. 그 관측이 연산보다 메모리에서 원인을 찾은 교재 결론의 근거입니다.
...
> **양자화가 어디서 버는지는 방식과 하드웨어가 정합니다.** 가중치만 줄이는 방식이면 이득은 메모리 쪽에, 계산까지 낮은 정밀도로 하는 방식이면 커널 쪽에 남습니다. 두 경우를 양자화라는 한 단어로 묶으면 원인을 잘못 짚게 됩니다.
지표만으로 보이지 않던 차이가 프로파일러에서 드러났습니다. 처리량과 ITL은 속도가 달라졌다는 사실까지만 보여 줍니다. 어느 경로가 빨라졌는지는 커널 시간을 나눈 뒤에야 알 수 있었습니다.
...
F1d에서 **0.998**까지 오른 KV 사용률도 Prometheus에는 **0.078**까지만 남았습니다. 피크가 두 스크랩 사이를 지나갔습니다.
> 짧은 버스트 측정은 **되짚어 찍기에 맞지 않습니다.** 대시보드에 증거를 남기려면 같은 대비를 **스크랩 주기보다 충분히 길게** 다시 실행해야 합니다.
세 상태를 3분씩 다시 실행했습니다. 새 결론을 더하기보다 이미 확인한 현상이 모니터링 화면에 어떻게 나타나는지를 기록했습니다.
<table fit-page-width="true" header-row="true">
<tr>
<td>구간</td>
<td>구성</td>
...
> **GPU 사용률 99%만으로는 효율과 수용 한계를 판단할 수 없습니다.** 같은 99%에서 처리량이 1,703이기도 하고 2,269이기도 했습니다. 사용률은 GPU가 노는지만 알려 주고 무엇을 하느라 바쁜지는 알려 주지 않습니다.

---

## 7. 이전 실험까지 같은 KV 예산 축에 놓았다
앞 그림의 가로축은 KV 예산입니다. BF16 스윕으로 만든 점선은 평평합니다. 가로축만 움직인 팔에서는 처리량이 달라지지 않았습니다. 선 위아래로 벗어난 팔에서만 다른 효과가 나타났습니다.
...
이 결과는 판단 절차도 바로잡았습니다. "조건이 달라졌으니 비교할 수 없다"고 말하려면 그 조건이 실제로 결과를 움직이는지 먼저 재야 합니다. 영향도 모른 채 비교에서 빼면 쓸 수 있는 정보까지 버리게 됩니다.

---

## 8. 운영에서는 예산, 제거 실험, ITL 순서로 확인한다
...
메모리가 실제로 모자란 신호는 `vllm:num_preemptions_total`입니다. 이 값이 오른다는 것은 vLLM이 이미 들어와 있던 요청을 쫓아내고 있다는 뜻이고 그때부터 처리량이 떨어집니다. 측정에서도 선점이 0인 동안에는 사용률이 99%든 44%든 처리량이 같았습니다. 선점이 난 지점에서만 35% 떨어졌습니다(6장의 대시보드에도 같은 신호가 남아 있습니다).
`DCGM_FI_DEV_GPU_UTIL`도 혼자서는 답을 주지 않습니다. 6장의 세 구간은 GPU 사용률이 전부 99%였는데 처리량은 224에서 2,269까지 열 배 차이가 났습니다. 사용률은 GPU가 노는지만 말합니다.
...
조건이 달라져 비교할 수 없다는 판단도 검증이 필요합니다. 그 조건의 영향을 한 번 재 보는 비용이 대개 더 쌉니다.
...
<td>FP8이 첫 시도에 떠서 다운로드 0으로 끝냈음. 다른 방식은 미측정</td>
	...
	세대가 바뀌면 수치의 크기는 달라져도 확인할 질문은 남습니다. 정밀도를 한 단계 더 낮추면 스텝마다 읽는 바이트가 더 줄고 하드웨어에 그 정밀도의 텐서코어가 있으면 커널까지 갈아탑니다. 5장에서 FP8이 그랬던 것과 같은 일이 한 번 더 일어나는 셈입니다. 그러니 FP4에서도 물어야 할 것은 같습니다. **이득이 KV 예산에서 왔는가, 아니면 가중치를 읽고 계산하는 경로에서 왔는가.** 3장의 제거 실험은 양자화 포맷이 달라져도 같은 방식으로 적용됩니다.
	측정 자체의 한계도 둘 있습니다.
...
read_conc() {
  kubectl -n llm-serving-lab logs deploy/vllm-baseline \
	...
	한 가지 짚어 둘 것이 있습니다. 6장에서 적었듯 예산 차이는 이 구간에서 처리량을 움직이지 않으므로 ±5%로 맞추지 않았어도 결론은 같았을 것입니다. 그래도 맞춘 이유는 제거 실험의 성격 때문입니다. 예산 차이가 원인일 수 있다는 반론을 데이터로 미리 확인하려면 예산이 실제로 중요한지와 무관하게 같은 값에 놓고 재는 편이 낫습니다.
	### 부록 B. 왜 FP8을 골랐나
...
5. S1 잔존 0건: 통과
6. 임의 비유·수사 추가 없음: 통과
등급: A — S1 잔존 0건, S2 잔존 2건, 변경률 10.79%, 자체검증 6/6
주요 변경:
- 결과 한눈에 보기 7행 표 → 핵심 관측 3개와 본문 실측표로 중복 제거
- 5-B 모니터링 장 → 6장으로 편입하고 이후 번호를 순서대로 정리
- 처리량 숫자만으로는 어디서 벌었는지 알 수 없습니다 → 처리량과 ITL의 역할을 분리해 직설형으로 정리
- 긴 실험 설명 → 조건·관측·해석 순서의 짧은 문단으로 재배치
-->
